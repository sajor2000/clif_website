...
// Site naming lives in siteNames.ts so the browser can import it too — this
// module cannot be bundled for the client (node:fs above). Re-exported here so
// existing importers are unaffected.
import { AGGREGATE_SITE, SITE_ORDER, compareSites } from './siteNames';
export { AGGREGATE_SITE, SITE_LABELS, SITE_ORDER, siteLabel, compareSites } from './siteNames';
...
export interface CohortDef {
  ...
  label: string;
  /** Longer wording surfaced as a title/tooltip where the label is compressed */
  tooltip?: string;
...
/**
 * Empty: the ICU / Non-ICU third-level picker was removed from the dashboard.
 *
 * Everything downstream derives from this array — the sub-select in the picker,
 * PANEL_COHORTS (and so which Summary/Outcomes panels are built), MapExplorer's
 * per-cohort counts — so emptying it removes the level everywhere at once.
 *
 * The CSVs under src/data/cohorts/<parent>__<group>/ were deleted in Aug 2026
 * and scripts/refresh-cohort-tableone.mjs no longer derives them (see
 * src/data/processing.md). Restoring the level means re-adding the sub-cohort
 * SUBS jobs to that script AND putting entries back here.
 */
