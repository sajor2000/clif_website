...
* `location_type` represents the **nursing capabilities of the unit** and should be derived from ADT data alone — do **not** map `location_type` based on who the attending clinician or service is.
...
- In the [lab categories mCIDE](https://github.com/clif-consortium/CLIF/blob/3.0/mCIDE/labs/clif_lab_categories.csv), an analyte that is valid for more than one panel or specimen has those values listed **slash-delimited** — `potassium` shows `bmp/poc`, `albumin` shows `lft/peritoneal_fluid_panel/pleural_fluid_panel`. The list is what the analyte *permits*, not what a given result *is*: emit one CLIF row per lab result and resolve each cell to the single applicable value. Do not create a row per slash-separated value.
- **Point-of-care labs.** POC measurements can differ in accuracy from the central lab, so CLIF keeps them separable without a separate `lab_category` — the panel does the work. A potassium resulted off a BMP is `lab_category = potassium` with `lab_order_category = bmp`; a POC/iSTAT potassium is the same `lab_category = potassium` with `lab_order_category = poc`. A study wanting only central-lab chemistry filters on `lab_order_category`; a study wanting every potassium ignores it. Seven analytes currently permit `poc`: `bicarbonate`, `creatinine`, `glucose`, `hematocrit`, `lactate`, `potassium`, and `sodium`. Bedside capillary glucometer glucose has its own panel — `lab_category = glucose` with `lab_order_category = fingerstick` — so it stays distinguishable from analyzer POC glucose.
- **Bicarbonate vs. CO2.** BMP bicarbonate (often reported as "CO2") is `bicarbonate` + `bmp`; HCO3⁻ off an ABG/VBG is `bicarbonate` + `blood_gas`. pCO2 maps to `pco2_arterial` / `pco2_venous`, never to `bicarbonate`. Total CO2 (tCO2, e.g. iSTAT TCO2) is not mapped.
- **Blood specimens.** For mapping purposes, we use **`plasma_blood`** as a broad category encompassing whole blood, plasma, and serum, while recognizing that individual labs may be performed using different blood components. Sites do not need to distinguish serum from plasma from whole blood — all three map to `plasma_blood`.
...
|-------------------|------------------------|-------------------------|-------------------------|-------------------------------|--------------------|------------------|-------------|----------|------------------|----------------|-------------------|----------------------|---------------|--------------|
| 1001014           | 2023-05-01 07:00:00+00:00 UTC | 2023-05-01 07:15:00+00:00 UTC | 2023-05-01 08:00:00+00:00 UTC | Complete blood count w/ diff  | cbc                | WBC              | wbc                  | 8.2      | 8.2              | 10^3/ul        | blood             | plasma_blood         | 6690-2        | 1.0          |
